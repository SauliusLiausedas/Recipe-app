...
class AddToMongodb extends Component {
    ...
    addToMongodb() {
        getRandomRecipes(1).then((data) => {
            console.log(fetch('recipe-app-mindgsv244709.codeanyapp.com:2000/insertfrommealdb', {
                method: 'POST',
                headers: {
    ...
    addCategoriesToMongo() {
        getCategories().then(categories => {
            categories.categories.forEach(category => {
                console.log(fetch('recipe-app-mindgsv244709.codeanyapp.com:2000/insertcategories', {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
